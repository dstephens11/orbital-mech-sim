...
def default_capture_orbit():
    ...
    return {
        ...
        "periapsis_radius_km": c.JOI_CAPTURE_PERIAPSIS_RJ * c.R_JUPITER,
        "apoapsis_radius_km": c.JOI_CAPTURE_APOAPSIS_RJ * c.R_JUPITER,
    }


def capture_model_summary():
    """Return a JSON-friendly summary of the default capture model."""
    orbit = default_capture_orbit()
    return {
...
def compute_jupiter_capture(v_sc_helio_kms, v_jupiter_helio_kms):
    """Compute Jupiter arrival ``v_inf`` and impulsive JOI into the default orbit."""
    capture_orbit = default_capture_orbit()
